		...
		if UnitAffectingCombat("player") then
			...
			if animalsTable.validAnimal() and GetTime() > animalsTable.throttleSlaying then
			    if animalsTable.cds then
			        ...
			        if not animalsTable.artifactWeapon.weaponPerks[195399] or animalsTable.artifactWeapon.weaponPerks[195399].currentRank == 0 then
				        if animalsTable.spellCanAttack(touch_of_death) then animalsTable.cast(_, touch_of_death, _, _, _, _, "Touch of Death") return end
				    else
				        if animalsTable.spellCanAttack(touch_of_death) and animalsTable.spellCDDuration(strike_of_the_windlord) < 8 and animalsTable.spellCDDuration(fists_of_fury) <= 3 and animalsTable.spellCDDuration(rising_sun_kick) < 8 then animalsTable.cast(_, touch_of_death, _, _, _, _, "Touch of Death") return end
			        end
		        	...
		        	if not animalsTable.artifactWeapon.weaponPerks[205320] or animalsTable.artifactWeapon.weaponPerks[205320].currentRank == 0 then
			        	...
			        	if animalsTable.talent73 and animalsTable.spellIsReady(serenity) and chi() > 1 and animalsTable.spellCDDuration(strike_of_the_windlord) < 7 and animalsTable.spellCDDuration(fists_of_fury) <= 3 and animalsTable.spellCDDuration(rising_sun_kick) < 8 then animalsTable.cast(_, serenity, _, _, _, _, "Serenity") return end
		        	end
			    end
			    ...
			    if animalsTable.artifactWeapon.weaponPerks[205320] and animalsTable.artifactWeapon.weaponPerks[205320].currentRank > 0 and animalsTable.spellCanAttack(strike_of_the_windlord) then animalsTable.cast(_, strike_of_the_windlord, _, _, _, _, "Strike of the Windlord") return end
